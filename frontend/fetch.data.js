...
async function fetchSongs() {
    try {
        const response = await fetch('http://localhost:7000/api/songs/all');
...
async function deleteSong(id) {
    if (confirm('Are you sure you want to delete this song?')) {
        try {
            const response = await fetch(`http://localhost:7000/api/songs/delete/${id}`, {
                method: 'DELETE'
            });
            if (response.ok) {
                fetchSongs();
...
async function updateSong() {
    ...
    try {
        const response = await fetch(`http://localhost:7000/api/songs/update/${id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
